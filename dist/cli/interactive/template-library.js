...
const resource_paths_1 = require("../utils/resource-paths");
class TemplateLibrary {
    ...
    constructor() {
        ...
        const resourcePaths = (0, resource_paths_1.getResourcePaths)();
        this.templatesPath = resourcePaths.templates;
        this.validator = new schema_1.ConfigurationValidator();
    }
    async loadTemplates() {
        try {
            const templateFiles = await fs_1.promises.readdir(this.templatesPath);
            const yamlFiles = templateFiles.filter(file => file.endsWith('.yaml') || file.endsWith('.yml'));
            for (const file of yamlFiles) {
                const templatePath = path_1.default.join(this.templatesPath, file);
     ...
     */
    async validateTemplate(templatePath, _filename) {
        try {
     ...
     */
    validateSimulationLogic(logic, parameters) {
        try {
            ...
            const usedVariables = [...cleanedLogic.matchAll(variablePattern)]
                .map(match => match[1])
                .filter(name => !['Math', 'random', 'const', 'let', 'var', 'return', 'if', 'else', 'for', 'while', 'function', 'true', 'false', 'null', 'undefined'].includes(name) &&
                !name.match(/^\d/) // not numbers
            );
            const undefinedVariables = usedVariables.filter(variable => !parameterNames.includes(variable) &&
    ...
    createBusinessTemplate(config, filename) {
    ...
    extractIndustryRelevance(config) {
    ...
    generateBusinessInsights(config) {
        const insights = [];
        const name = config.name.toLowerCase();
        if (name.includes('restaurant')) {
            insights.push('Food costs should typically stay below 32% of revenue', 'Labor costs generally range 25-35% of revenue depending on service model', 'Location quality has 2-3x impact on customer traffic and revenue', 'Table turnover rate directly impacts revenue per seat capacity');
        }
        else if (name.includes('saas') || name.includes('growth')) {
            insights.push('Net Revenue Retention >100% indicates healthy expansion revenue', 'CAC Payback Period should be <12 months for efficient growth', 'Monthly churn compounds exponentially - small improvements have big impact', 'Product-led growth typically has lower CAC but requires higher product investment');
        }
        else if (name.includes('marketing')) {
            insights.push('Multi-channel campaigns typically achieve 1.2-1.5x higher ROI than single-channel', 'Customer lifetime value must exceed acquisition cost by 3:1 minimum', 'Organic amplification from paid campaigns often provides 20-40% bonus reach', 'Brand impact compounds over time - consider long-term value beyond immediate ROI');
        }
        else if (name.includes('software') || name.includes('project')) {
            insights.push('Team scaling follows Brooks\' Law - doubling team size rarely halves delivery time', 'Technical complexity has exponential impact on delivery timeline', 'Quality requirements significantly impact both timeline and cost', 'Risk buffers should increase with architectural complexity and team experience gaps');
        }
        else if (name.includes('ecommerce') || name.includes('conversion')) {
            insights.push('Mobile conversion rates typically 30-50% lower than desktop', 'Checkout funnel optimization often provides highest ROI improvements', 'Customer lifetime value improvements compound through retention', 'Personalization can improve conversion rates 10-30% but requires significant investment');
        }
        else if (name.includes('manufacturing')) {
            insights.push('Overall Equipment Effectiveness (OEE) >85% indicates world-class operations', 'Capacity utilization >90% often creates bottlenecks and quality issues', 'Automation ROI depends heavily on product complexity and volume', 'Lead time reduction often has greater business impact than cost reduction');
        }
        return insights;
    }
    extractIndustryBenchmarks(config) {
        const benchmarks = {};
        const name = config.name.toLowerCase();
        if (name.includes('restaurant')) {
    ...
    generateBusinessIntelligence(config) {
        const name = config.name.toLowerCase();
        const tags = config.tags || [];
        const hasARR = config.parameters.some(p => p.key.toLowerCase().includes('annualrecurringrevenue'));
        ...
        const hasLongTermMetrics = config.parameters.some(p => p.key.toLowerCase().includes('lifetime') ||
        ...
        if (name.includes('marketing'))
            keywords.push('marketing', 'campaign', 'acquisition', 'roi');
        if (name.includes('investment'))
            keywords.push('investment', 'roi', 'payback', 'npv');
        if (name.includes('team') || name.includes('scaling'))
            keywords.push('team', 'scaling', 'hiring', 'growth');
        if (name.includes('software'))
            keywords.push('software', 'technology', 'productivity', 'automation');
        if (hasARR)
            keywords.push('arr', 'saas', 'recurring', 'subscription');
        ...
        if (arrParam)
            parameterPriority.push(arrParam.key);
        const budgetParams = config.parameters.filter(p => p.key.toLowerCase().includes('budget') ||
    ...
    searchTemplates(query) {
        ...
        return Array.from(this.templates.values()).filter(template => template.info.name.toLowerCase().includes(searchTerm) ||
    ...
    generateBusinessGuidance(template) {
        const guidance = [
            `📋 ${template.info.name}`,
            '',
